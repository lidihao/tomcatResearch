...
public class NioEndpoint extends AbstractEndpoint<NioChannel> {
     ...
     */
    /**
     * The NIO connector implements an OutOfMemoryError strategy called parachute.
     * It holds a chunk of data as a byte array. In case of an OOM, this chunk of data is released
     * and the error is reported. This will give the VM enough room to clean up. The oomParachute
     * represents the size in bytes of the parachute(the byte array). The default value is1024*1024(1MB).
     * Please note, this only works for OOM errors regarding the Java Heap space, and there is absolutely no
     * guarantee that you will be able to recover at all. If you have an OOM outside of the Java Heap,
     * then this parachute trick will not help.
     */
    //tomcat用来恢复oom的问题，预先分配一段空间，当系统出现问题的时候，释放该空间
    //用来帮助jvm恢复，不至于停止
    ...
    protected ConcurrentLinkedQueue<NioChannel> nioChannels = new ConcurrentLinkedQueue<NioChannel>() {
        ...
        public boolean offer(NioChannel socket) {
            //检查是否超过bufferPool和bufferPoolsize的数量
    ...
    public Poller getPoller0() {
        //循环使用轮询器
        int idx = Math.abs(pollerRotater.incrementAndGet()) % pollers.length;//直接自增pollerRotater
        return pollers[idx];
    }
     ...
     */
    //acceptor将socket添加到poller
    protected boolean setSocketOptions(SocketChannel socket) {
        // Process the connection
        try {
            //disable blocking, APR style, we are gonna be polling it
            socket.configureBlocking(false);//设为非阻塞模式
            Socket sock = socket.socket();
            socketProperties.setProperties(sock);//设置socket的各项属性
            //从缓存池中取NioChannel
            NioChannel channel = nioChannels.poll();
            if ( channel == null ) {
                // SSL setup
                if (sslContext != null) {
                    SSLEngine engine = createSSLEngine();//创建sslengine
    ...
    public boolean processSocket(NioChannel socket, SocketStatus status, boolean dispatch) {
        try {
            ...
            if ( dispatch && getExecutor()!=null ) getExecutor().execute(sc);//调用线程处理socket
            else sc.run();
        ...
        public void run() {
            ...
            while (running) {
                ...
                try {
                    ...
                    //最大连接数加1，当最大连接数超过limit,accept线程挂起
    ...
    public class Poller implements Runnable {
        ...
        protected void destroy() {
            ...
            close = true;
            selector.wakeup();//唤醒seletor要他处理关闭的事情
        }
        ...
        public void addEvent(Runnable event) {
            events.offer(event);//添加事件
            if ( wakeupCounter.incrementAndGet() == 0 ) selector.wakeup();//唤醒selector,处理evetns
        }
                ...
                try {
                    ...
                    try {
                         ...
                         * 作用一，帮助Poller选择select方法
                         在run执行时，当前已经有了5个channel注册到Poller上，所以wakeupCounter.getAndSet(-1) > 0 条件满足，Poller调用selector的非阻塞模式的select方法被调用
                         作用二，让当前阻塞的select方法立即返回
                         在run执行时，如果当前没有channel注册到Poller上，wakeupCounter.getAndSet(-1) > 0 条件不满足，但wakeupCounter的值已经被设为-1了。Poller调用阻塞的select方法。在这期间，如果有新的channel注册进来，则 wakeupCounter.incrementAndGet() == 0条件满足，select.wakeup方法被调用，让selector.select(selectorTimeout)方法立即返回。
                    ...
                    while (iterator != null && iterator.hasNext()) {
                        ...
                        if (attachment == null) {
                            ...
                            attachment.access();
                            iterator.remove();
                            processKey(sk, attachment);//处理key
                        }
                    }//while

                    //process timeouts
                    timeout(keyCount,hasEvents);
                    if ( oomParachute > 0 && oomParachuteData == null ) checkParachute();//检查parachuteData是否已经分配了
                } catch (OutOfMemoryError oom) {
                    try {
                        oomParachuteData = null;//释放该空间
                        releaseCaches();//清空缓存
                        log.error("", oom);
                    }catch ( Throwable oomt ) {
                        try {
            ...
            try {
                if ( close ) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {//处理请求文件的请求
                            processSendfile(sk,attachment, false);
                        } else {
                            if ( isWorkerAvailable() ) {
                                //取消socket感兴趣的事件，因为现在正在处理他们，防止重复处理该socketKey,这里很重要否则，会导致同时多个线程处理一个请求，导致内存迅速升高
                                unreg(sk, attachment, sk.readyOps());
                                boolean closeSocket = false;
                                // Read goes before write
                                if (sk.isReadable()) {
                                    if (!processSocket(channel, SocketStatus.OPEN_READ, true)) {//处理读请求
                                        closeSocket = true;
                                    }
                                }
                                if (!closeSocket && sk.isWritable()) {
                                    if (!processSocket(channel, SocketStatus.OPEN_WRITE, true)) {//处理写请求
                                        closeSocket = true;
                                    }
                                }
                                if (closeSocket) {
        ...
        protected void timeout(int keyCount, boolean hasEvents) {
            ...
            //并不是没次都处理timeouts,每次都处理,poller的负荷太大，当以下条件成立时才处理:
            //- selector超时,keyCount=0（说明此时poller的负载不是很重)
            //- nextExpiration到了
            //- server socket被关闭

            //当keyCount>0||hasEvents=true，poller还有其他重要的事情要干，不能处理timout
                ...
                for (Iterator<SelectionKey> iter = keys.iterator(); iter.hasNext();) {
                    ...
                    try {
                        ...
                        if ( ka == null ) { //什么时候ka为空？
                            ...
                            //以上两种为socket出错的情况
                        } else if (ka.isComet() && ka.getCometNotify() ) {  //comet还不是恒了解
                        ...
                        } else if (ka.isAsync() || ka.isComet()) {//sevlet将socket设置成async
                            if (close) {
                                key.interestOps(0);
                                ka.interestOps(0); //avoid duplicate stop calls
                                processKey(key,ka);
...
//            System.out.println("timeout completed: keys processed=" + keycount +
...
// ----------------------------------------------------- Key Attachment Class
     ...
     */
    //一个任务，由executor执行，根据tcp状态，确定socketStatus
    protected class SocketProcessor implements Runnable {
        ...
        public void run() {
            ...
            if (ka != null && ka.isUpgraded() &&
                ...
                synchronized (socket) {
                    //try {
                   //     Thread.sleep(Integer.MAX_VALUE);  //这段是自己加的
                    //}catch (InterruptedException e){
        ...
        private void doRun(SelectionKey key, KeyAttachment ka) {
            try {
                ...
                try {
                    if (key != null) {
                        ...
                        if (socket.isHandshakeComplete() ||
                            ...
                            handshake = socket.handshake(
                                    key.isReadable(), key.isWritable()); //ssl handshake返回注册事件
                    ...
                    handshake = -1;
                    System.err.println("Error during SSL handshake"+x);
                }catch ( CancelledKeyException ckx ) {
                    handshake = -1;
                }
                if ( handshake == 0 ) {
