 ...
 * @since 6.0
 */
//有两种模式，share模式与unshare模式，share模式就是调用NioSelector的selector来进行读写。
    //unshare模式是每次从selectorPool中取一个selector，将socket注册到该selector进行读写
public class NioSelectorPool {
    ...
    //获取一个sharedSelector,采用懒汉模式
    protected Selector getSharedSelector() throws IOException {
        //double check:线程安全，并提高性能
        if (SHARED && SHARED_SELECTOR == null) {
            synchronized ( NioSelectorPool.class ) {
                if ( SHARED_SELECTOR == null )  {
                    //Selector.open()不是线程安全的，要加类锁，保证一次只有一个selector打开
    ...
    public int write(ByteBuffer buf, NioChannel socket, Selector selector,
        ...
        //如果是share模式并且是block，调用blockingSelector进行读写
        if ( SHARED && block ) {
        ...
        try {
            ...
            while ( (!timedout) && buf.hasRemaining() ) {
                ...
                if ( keycount > 0 ) { //only write if we were registered for a write
                    ...
                    if (cnt==0 && (!block)) break; //don't block 不是阻塞读取，退出
                }
                if ( selector != null ) {
                    //阻塞读写，将scoket注册到selector上
