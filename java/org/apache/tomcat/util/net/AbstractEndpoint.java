 ...
 */
//模版模式,实现Endpoint的生命周期,线程池,Acceptor的创建,unbind,bind会让子类实现
public abstract class AbstractEndpoint<S> {
     ...
     * is 100.
     */
    private int backlog = 100; //设置连接请求队列的长度
    ...
    //如果是connector内置线程池，则返回maxThread,否则返回-1;
...
//-----------------------------------------以上是一些get set方法---------------------------
   //开启线程池
    public void createExecutor() {
        ...
        TaskQueue taskqueue = new TaskQueue();//阻塞队列
    ...
    public void shutdownExecutor() {
        if ( executor!=null && internalExecutor ) {
            if ( executor instanceof ThreadPoolExecutor ) {
                ...
                tpe.shutdownNow();//这里通过thread.interrupt()来停止所有正在运行的线程，并移除那些等待执行的任务。如果那些通过interrupt（）都不能
                                  //停掉的线程，线程池将进入Stop状态
                long timeout = getExecutorTerminationTimeoutMillis();
                if (timeout > 0) {
                    try {
                        tpe.awaitTermination(timeout, TimeUnit.MILLISECONDS);//等待timeout秒,等待线程池完全关闭
