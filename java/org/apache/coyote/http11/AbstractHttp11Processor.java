...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
    ...
    public final void action(ActionCode actionCode, Object param) {
        ...
        case CLIENT_FLUSH: {
            try {
                getOutputBuffer().flush();//将信息刷新到浏览器
            } catch (IOException e) {
                setErrorState(ErrorState.CLOSE_NOW, e);
        ...
        while (!getErrorState().isError() && keepAlive && !comet && !isAsync() &&
            ...
            try {
                ...
                if (!getInputBuffer().parseRequestLine(keptAlive)) { //解析请求行
                    if (handleIncompleteRequestLineRead()) {  //处理没完成的解析请求行的事件
                        break;
                    }
                }
                ...
                if (endpoint.isPaused()) {
                    ...
                    if (!getInputBuffer().parseHeaders()) { //解析请求头，非阻塞读取
            ...
            if (!getErrorState().isError()) {
                try {
                    ...
                    adapter.service(request, response);//调用adapter处理请求
    ...
    public void endRequest() {
        ...
        // Finish the handling of the request //结束请求
        if (getErrorState().isIoAllowed()) {
            try {
